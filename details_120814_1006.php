...
<?php $straddtofavorites=dbQuery("SELECT  * FROM tbl_savetofavorites where property_id=".$addtonote_property_id." and user_id=".$_SESSION['user']['id']);
...
<div class="clear"></div>    
    <span style="display:block;position:relative;top:-36px;text-align:right;margin-bottom:-15px;"><a href="#"><span>View all schools in Haringey</span></a></span>
				 ...
				 ?>
					...
					<?php }else{?>
					 ...
					 <?php while($property_rows=dbFetchArray($tot_property_num)){?>
					   <tr>   
					   <td><div style="width:180px; height:40px; border:1px solid #CCC; margin:2px 0 0 0; padding:7px;"><?php echo $property_rows['property_note'];?></div> </td>  
					   <td><a href="details.php?source=<?php echo base64_encode($source)?>&proid=<?php echo base64_encode($proid)?>&mode=edit&id=<?php echo $property_rows['id']?>"><div style="width:auto; height:auto; background:#CCC; padding:7px;">Edit</div></a></td>  
					   <td><a href="details.php?source=<?php echo base64_encode($source)?>&proid=<?php echo base64_encode($proid)?>&mode=delete&id=<?php echo $property_rows['id']?>"><div style="width:auto; height:auto; background:#CCC; padding:7px; margin:0 0 0 2px;">Delete</div></a></td>  
					   </tr>
					 <?php }?>   
					 </table>
