<?php
	 ...
	 */
	$aColumns = array( 'user_first_name', 'user_address', 'postcode');
...
/*$sql_details = array(
	...
	if ( $_GET['sSearch'] != "" )
	{
		$sWhere = "WHERE ( UPPER( user_type_agent ) = UPPER('YES') ) AND ( ";
		...
		$sWhere .= ')';
	} else {		$sWhere = " WHERE UPPER( user.user_type_agent ) = UPPER( 'YES' )";	}
	...
	$sQuery = "
		SELECT SQL_CALC_FOUND_ROWS  user.user_first_name AS user_first_name , cities.name AS user_address,area.postcode AS postcode
		FROM user INNER JOIN cities ON user.state_id = cities.state_id INNER JOIN area
		$sWhere
		$sOrder
		$sLimit
	";
