...
if($_GET['rate1']!=''){
	$strProperty=dbFetchArray(dbQuery("SELECT  AVG(property_total_price) as averageCityPrice FROM property where city_id='".$_GET["cid"]."' and property_furnishing='".$_GET["rate1"]."' and property_status='Active' and property_transaction_type='sell' and property_change_status='Sold'"));
echo "&pound; ".number_format($strProperty['averageCityPrice']);
	...
	$currentValue=dbFetchArray(dbQuery("SELECT AVG(property_total_price) as avgPrice FROM property where property_created_date<=$end_date and  city_id='".$_GET["cid"]."' and property_status='Active' and property_transaction_type='sell' and property_change_status='Unsold' and property_status='Active'"));
