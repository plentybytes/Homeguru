...
if($action=='sale')
{
	...
	if(!empty($_REQUEST["property_type"]))
	{
		...
		if(!empty($where))
	       $where.=" and property_category_id='$propertyType'";
	      else
	...
	if(!empty($_REQUEST["bedroom"]))
	{
		...
		if(!empty($where))
	       $where.=" and property_bedrooms='$bedroom'";
	     else
	       $where.=" property_bedrooms='$bedroom'";
	        
	
	
	}
	
	if(!empty($_REQUEST["minimum_price"]) && !empty($_REQUEST["maximum_price"]))
	{
		...
		if(!empty($where))
	      $where.=" and property_total_price between  '$minimum' and '$maximum'";
	     else
...
if(!empty($_REQUEST["beds_min"]) && !empty($_REQUEST["beds_max"]))
{
	  if(!empty($where))
	    $where.=" and property_bedrooms between  '".$_REQUEST["beds_min"]."' and '".$_REQUEST["beds_max"];
       else
...
if(!empty($_REQUEST["posting"]))
{
	...
	if(!empty($where))
	  $where.=" and property_created_date >= '$end_date'";  
     else
      $where.=" property_created_date >= '$end_date'";  
      

}
if(!empty($_REQUEST["shorting"]))
{
...
{
   if(!empty($where))
	  $where.=" and property_description LIKE '%".$_REQUEST["keywords"]."%'";  
     else
      $where.=" property_description LIKE '%".$_REQUEST["keywords"]."%'"; 
...
// changed here at 3/Mar/2014 by Stephin
    ...
    $strProperty="SELECT  * FROM property where (property_postal_code LIKE '%"
    .trim($_REQUEST["location"])."%' or  property_address LIKE '%"
    .$_REQUEST["location"]."%') and property_transaction_type='sell' 
    and property_status='Active'  and (".$where.") $orderBy";
    //echo $strProperty;
...
if($_REQUEST["shorting"]!='')
{
	...
	//$strProperty = "SELECT  * FROM property where city_id='".$_REQUEST["location"]."' and property_transaction_type='rent' and property_status='Active'  and (".$where.") $orderBy";
    ...
    if(!empty($_REQUEST["keywords"]))
{
   if(!empty($where))
     ...
     $strProperty="SELECT  * FROM property where (property_postal_code LIKE '%"
    .trim($_REQUEST["location"])."%' or  property_address LIKE '%"
    .$_REQUEST["location"]."%') and property_transaction_type='rent' 
    and property_status='Active'  and (".$where.") $orderBy";	
