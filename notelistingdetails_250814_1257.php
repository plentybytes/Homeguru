...
foreach($_POST['option2'] as $key=>$val){
  $tot_property_query=dbQuery("delete from tbl_property_note where property_id=".$val." and user_id=".$_SESSION['user']['id']);
}
}
...
$sql="SELECT * FROM(SELECT C.*,D.property_total_price,D.property_description,D.property_images FROM tbl_property_note AS C,
(SELECT  A.*,B.property_images FROM property as A LEFT OUTER JOIN property_images as B ON A.property_id=B.property_id) 
...
<div class="list_part">
	...
	<div class="list_part_box">
		<div class="list_part_box_left"><input id="<?php echo $resultProperty['property_id']?>" name="option2[]" type="checkbox" value="<?php echo $resultProperty['property_id']?>" class="second" /> 
		<span><img src="images/property_images/<?php echo $resultProperty['property_images']?>" width="93" height="66" alt="img" /></span></div>
			...
			<div class="list_part_box_right"><img src="images/user_images/<?php echo $resultLogoProperty['logo']?>" width="100" height="69" alt="img" /></div>
			<?php }?>
		</div>
