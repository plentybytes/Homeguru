<?php error_reporting(0);require("../includes/application.php");
	...
	$siteName=SITE_NAME;
			...
			elseif($action=="register"){
				if(isset($_POST["_method"]) && ($_POST["_method"] == "access request")){
					...
					if(checkDuplicateUserName($userName) == '1'){
						$sqlDataArray=array("user_name" => $userName, "user_first_name" => $name, "verified_code" => $verifyCode, "receive_mail" => $check, "hear_about_us" => $hearAbout, "user_email" => $userEmail, "user_password" => $userPassword, "user_registration_date" => "NOW()", "user_status" => 'Deactive');
						...
						$messageStack->addMessageSession("Thanks for signup with us verify your mail address.", "success");
						...
						$subjects="Conformation link to activate your account";
						$messages="<html>
									<head>
									<title>Conformation link to activate your account</title>
									</head>
										<body>
											<table width='100%' cellpadding='2' cellspacing='4' border='0' style='border:1px dashed #009966;'>
												<tr>
									  ...
									  <tr>
										<td style='font-family:Arial, Helvetica, sans-serif; font-size:13px;'>Conformation link to activate your account</td>
									  </tr>
									  <tr>
						...
						$header = "From:  Conformation link <$from>\n";
						$header .= "MIME-Version: 1.0\n";
						$header .= "Return-Path: $siteName<$from>\n";
						$header .= "Content-Type: text/html;\n";
						$header .= "X-Mailer: PHP/" . phpversion();
						mail($userEmail,$subjects,$messages,$header);
						$messageStack->addMessageSession("Thanks for join us. conformation link send to your mail address.", "success");
			...
			elseif($action=="change-password"){
				if(isset($_POST["_method"]) && ($_POST["_method"]=="password request")){
					...
					$userPasswordQuery=dbQuery("SELECT * from  user WHERE user_id ='".$userId."' AND user_password ='".$requestCurrentPassword."' AND user_status ='Active'");
						if(dbNumRows($userPasswordQuery)==1){
...
";
			...
			elseif($action=="property_step_one"){
				if(isset($_POST["_method"]) && ($_POST["_method"]=="Add Property")){
					...
					$stateId=dbPrepareInput($_POST["state_id"]);
					$cityId=dbPrepareInput($_POST["city_id"]);
                                                                        $cityId=dbPrepareInput($_POST["addr"]);
					...
					$verifyCode=md5(uniqid(rand()));
					$sqlDataArray=array("user_id" => $userId, "property_owner_type" => $ownerType,"property_change_status" => $proChangeType, "property_area" => $propertyArea, "property_category_id" => $propertyCategoryId, "property_transaction_type" => $propertyTransactionType, "property_type" => $propertyType, "state_id" => $stateId,  "city_id" => $cityId, "property_total_price" => $propertyTotalPrice, "property_bedrooms" => $propertyBedrooms, "property_bathrooms" => $propertyBathrooms, "property_floor_number" => $propertyFloorNumber, "property_description" => $propertyDescription,  "property_created_date" => "NOW()");
			...
			$propertyId=dbPrepareInput($_POST["source"]);
				if(isset($_POST["_method"]) && ($_POST["_method"]=="Add Step Two") && (base64_decode($_POST["source"])==$_SESSION["source1"])){
					...
					$userId=$_SESSION['user']['id'];
				             if(count($_POST["amenities"])) { $amenties=implode(",",$_POST["amenities"]); }
					$propertyAmenties=dbPrepareInput($amenties);
					...
					$youtube=dbPrepareInput($_POST["yourtube"]);
					$postalCade=dbPrepareInput($_POST["postal_cade"]);
					//echo base64_decode($propertyId);die;
					$sqlDataArray=array("property_longitude" => $propertylongitude,"youtube"=>$youtube, "property_latitude" => $propertylatitude,"property_total_floor_number" => $propertyTotalFloorNumber,"property_construction" => $propertyConstruction, "property_furnishing" => $propertyFurnishing, "property_directional_facing" => $propertyDirectionalFacing,  "property_ownership_type" => $propertyOwnershipType, "	property_amenties" => $propertyAmenties, "property_landmarks" => $landmarks);	
					//print_r($sqlDataArray); die;		
