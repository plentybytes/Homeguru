	...
	$siteName=SITE_NAME;
			...
			if($action=="login"){
				if(isset($_POST["_method"]) && ($_POST["_method"] == "authentication process")){
					...
					/* $checkUserQuery=dbQuery("SELECT * from  user WHERE user_email ='".dbInput($loginEmail)."' or user_name ='".dbInput($loginEmail)."' AND user_password ='".dbInput($loginPassword)."' AND user_status ='Active'");*/
						$checkUserQuery=dbQuery("SELECT * from  user WHERE user_email ='".dbInput($loginEmail)."' AND user_password ='".dbInput($loginPassword)."' AND user_status ='Active'");
			...
			elseif($action=="register"){
				if(isset($_POST["_method"]) && ($_POST["_method"] == "access request")){
					...
					if($_POST['check']=="Yes")
					$homesguru_information_receive_status=1;
					else
					$homesguru_information_receive_status=0;
					
					if(checkDuplicateEMail($userEmail) == '1' ){
						/*code by akash to make useremail as username*/
					//if(checkDuplicateUserName($userName) == '1'){
					if(checkDuplicateUserName($userEmail) == '1'){
						$sqlDataArray=array("user_name" => $userEmail, "user_first_name" => $name, 
						"verified_code" => $verifyCode,"comp_name" => $comp_name, "logo" => $logo, 
						"receive_mail" => $check, "hear_about_us" => $hearAbout, "user_email" => $userEmail, 
						"user_password" => $userPassword, 
						"user_registration_date" => "NOW()", "user_status" => 'Deactive',"homesguru_information_receive_status"=>$homesguru_information_receive_status);
					//print_r($sqlDataArray);
                       // exit;
                        ...
                        $sqlDataArray=array("user_id" => $userEmail, "user_last_login" => "NOW()", "user_number_log" => '1');
                        dbPerform("user_logs", $sqlDataArray);
						$messageStack->addMessageSession("Thanks for signing up with us, Please verify your email address. Check your email.", "success");
...
";
			...
			elseif($action=="property_step_one"){
				if(isset($_POST["_method"]) && ($_POST["_method"]=="Add Property")){
					...
					$propertyDescription=dbPrepareInput(htmlspecialchars($_POST["property_description"]));
					...
					if($_POST['newhome']=="new")
					  $new_home_status="new";
					 else
					  $new_home_status="old"; 
					
					
					$sqlDataArray=array("user_id" => $userId, "property_owner_type" => $ownerType,
					"house_no" => $propertyHouse,"property_change_status" => $proChangeType, 
					"property_area" => $propertyArea, "property_category_id" => $propertyCategoryId, 
					"property_transaction_type" => $propertyTransactionType, "property_type" => $propertyType, 
					"state_id" => $stateId,  "city_id" => $cityId, "property_total_price" => $propertyTotalPrice,
					"property_postal_code" => $postcode,"postal_id"=>$postal_id, "property_address" => $propertyAddress,  
					"property_bedrooms" => $propertyBedrooms, "property_bathrooms" => $propertyBathrooms, 
					"property_floor_number" => $propertyFloorNumber, "property_description" => $propertyDescription,  
					"property_created_date" => "NOW()","new_home_status"=>$new_home_status);
                    //echo $new_addr[0];
                    //print_r($sqlDataArray);
                    //exit;
			...
			elseif($action=="edit_step_one"){
				...
				if(isset($_POST["_method"]) && ($_POST["_method"]=="Add Property")){
					...
					$sqlDataArray=array("property_owner_type" => $ownerType, 
					"property_area" => $propertyArea, "property_category_id" => $propertyCategoryId, 
					"property_transaction_type" => $propertyTransactionType, "property_type" => $propertyType, 
					"state_id" => $stateId,  "city_id" => $cityId, "property_total_price" => $propertyTotalPrice, 
					"property_bedrooms" => $propertyBedrooms, "property_bathrooms" => $propertyBathrooms, 
		...
		//	prperty Steping two Start Here 
			...
			elseif($action=="agent_contact_form"){
					if(isset($_POST["_method"]) && ($_POST["_method"]=="agent contact")){
						...
						//echo $content;	die;
						sendMail($userResult["user_first_name"], $userResult["user_email"], $subject, $content, SITE_OWNER, $from);
					sendMail($msgBodyName, $msgBodyEmail, $VisiterSubject, $VisiterContent, SITE_OWNER, $from);
					sendMail($msgBodyName, $from, $adminSubject, $adminContent, SITE_OWNER, $from);
